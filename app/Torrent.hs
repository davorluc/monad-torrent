...
import Crypto.Hash.SHA1 as SHA1 (hash)
...
import Decoder (DecodedValue, decodedToByteString, decodedToDictionary, sortInfo, toBencodedByteString)
import Prelude
...
makeQuery :: Map ByteString DecodedValue -> Prelude.String
makeQuery json = do
  let info = decodedToDictionary (json ! B.pack "info")
  let announce = decodedToByteString $ json ! B.pack "announce"
  let length = B.pack $ show $ info ! B.pack "length"
  let infoHash = generateURLEncodedInfoHash $ sortInfo info
  B.unpack $ B.concat [announce, B.pack "?info_hash=", infoHash, B.pack "&peer_id=12349679991234567890&port=6881&uploaded=0&downloaded=0&left=", length, B.pack "&compact=1"]

peersToAddressList :: ByteString -> [ByteString]
...
addressToIPAndPort :: ByteString -> ByteString
addressToIPAndPort address = do
  B.concat [B.intercalate (B.singleton '.') $ Prelude.map (B.pack . show . ord) (B.unpack ip), B.singleton ':', B.pack $ parsePort port]
  where
    (ip, port) = B.splitAt 4 address

parsePort :: ByteString -> Prelude.String
parsePort port = show $ ord (B.head port) * 256 + ord (B.last port)
